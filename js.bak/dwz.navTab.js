...
var navTab = {
	...
	//关闭选项卡
	//index:要关闭的选项卡，openTabid:要显示的选项卡
	_closeTab: function(index, openTabid){
		...
		//更新地址栏#号后面的参数
		/*
		console.log(this._getTabs().eq(this._currentIndex));
		var sCurrentUrl = window.location.href;
		var sNewTab = sCurrentUrl.indexOf('#');
		if(sNewTab !== -1){
			var sLocation = sCurrentUrl.substring(0,sCurrentUrl.indexOf('#'));
			var sNewUrl = this._getTabs().eq(this._currentIndex).find('>a').attr('href');
			window.location.href = sLocation + sNewUrl;
		}
		*/
	},
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			var $more = this._getMoreLi().eq(iOpenIndex);
			//console.log($more);
			...
			$more.find(">a").attr({"title": op.title, 'href': '#' + tabid}).text(op.title);
