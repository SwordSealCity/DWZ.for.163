...
transTime:function(dateFmt){
return new Date(parseInt(this.substring(6))).formatDate(dateFmt||'yyyy-MM-dd HH:mm:ss');}});})(jQuery);
function Map(){
...
String.prototype.transDate=function(dateFmt){
return new Date(parseInt(this.substring(6))).formatDate(dateFmt||'yyyy-MM-dd HH:mm:ss');}
...
var tabUrl='main';}else{
var tabUrl=$tab.attr('tabid');}
var $tree=this._treeBox;
$('a',$tree).each(function(){
if($(this).attr('rel')==tabUrl){
$("div.selected",$tree).removeClass('selected');
$(this).parent('div').addClass('selected');}else if(tabUrl=='main'){
$("div.selected",$tree).removeClass('selected');
$('>li',$tree).find('div').eq(0).addClass('selected');}});}};(function($){
$.extend($.fn,{
...
arTree._treeBox==null&&(arTree._treeBox=$this);
...
var isNewTab=_locaHref.indexOf('#');
var _newUrl=_locaHref.substring(_locaHref.indexOf('#')+1);
if(isNewTab!==-1&&_newUrl!==''){
navTab.openTab(_newUrl,_newUrl);}},10);}
function initLayout(){
var iContentW=$(window).width()-(DWZ.ui.sbar?$("#sidebar").width():34);
var iContentH=$(window).height()-$("#header").height()+3;
$("#container").width(iContentW);
$("#container .tabsPageContent").height(iContentH-6).find("[layoutH]").layoutH();
...
if($.fn.ajaxJsonTodo)$("a[target=ajaxJsonTodo]",$p).ajaxJsonTodo();
...
return this.componentBox.width()-63;},
...
var currentTabid=this._getTabs().eq(this._currentIndex).find('>a').attr('href');
if($.History){
setTimeout(function(){
$.History.addHistory(currentTabid,function(currentTabid){
var i=navTab._indexTabId(currentTabid);
if(i>=0)navTab._switchTab(i);},currentTabid);},10);}},
...
updateTitle:function(iTitle,url){
if(this._reloadTab){
this._reloadTab.find('>a').eq(0).attr({class:url,title:iTitle,href:url}).find('>span').text(iTitle);
this._getMoreLi().eq(this._reloadTab.index()).find('>a').attr({class:url,title:iTitle,href:url}).text(iTitle);}else{
this._getTabs().eq(this._currentIndex).find('>a').eq(0).attr({class:url,title:iTitle,href:url}).find('>span').text(iTitle);
this._getMoreLi().eq(this._currentIndex).find('>a').attr({class:url,title:iTitle,href:url}).text(iTitle);}},
openTab:function(tabid,url,options){
var op=$.extend({title:"加载中…",data:{},fresh:true,external:false},options);
...
$tab.find(">a").attr({"title":op.title,'href':'#'+tabid}).find(span$).text(op.title);
...
var tabFrag='<li tabid="#tabid#"><a href="##tabid#" title="#title#" class="#tabid#"><span>#title#</span></a><a href="javascript:;" class="close">close</a></li>';
...
this._moreBox.append('<li><a href="##tabid#" title="#title#">#title#</a></li>'.replaceAll("#tabid#",tabid).replaceAll("#title#",op.title));
var $tabs=this._getTabs();
...
thead.wrap("<div class='gridHeader'><div class='gridThead'><table style='width:"+(tlength-30)+"px;'></table></div></div>");
...
tbody.wrap("<div class='gridScroller'"+layoutStr+" style='width:"+$tc.width()+"px;'><div class='gridTbody'><table style='width:"+(tlength-30)+"px;'></table></div></div>");
...
return offset;},
resetW:function(tb1,tb2,oldW,newW){}};})(jQuery);(function($){
$.fn.extend({
...
success:$callback,
error:DWZ.ajaxError});}
function ajaxJsonTodo(obj,url,callback){
...
alertMsg.confirm(title,{
okCall:function(){
ajaxJsonTodo($this,url,$this.attr("callback"));}});}else{
ajaxJsonTodo($this,url,$this.attr("callback"));}
event.preventDefault();});});},
dwzExport:function(){
...
function _doPost(){
$.ajax({
type:'POST',url:$this.attr('href'),contentType:'application/json; charset=utf-8',dataType:'json',cache:false,
data:function(){
if(postType=='map'){
return $.map(ids.split(','),function(val,i){
return{name:selectedIds,value:val};})}else if(postType=='json'){
return '{"ids":"'+ids+'"}';}else{
var _data={};
...
options.css({height:"300px","overflow-y":"scroll"});}
