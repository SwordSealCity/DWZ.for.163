...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			var tbody_id = typeof tbody.attr('id') == undefined ? tbody.attr('id'): 'tbody_id';
			var layoutStr = layoutH ? " layoutH='" + layoutH + "'" : "";
			
			tbody.wrap("<div class='gridScroller'" + layoutStr + " style='width:" + $tc.width() + "px;'><div class='gridTbody'><table id='"+ tbody_id +"' style='width:" + (tlength - 20) + "px;'></table></div></div>");
			...
			*/
			...
			$(">tr", thead).each(function(){
				$(">th", this).each(function(i){
					var th = this, $th = $(this);
					// mouseover Start
					$th.live('mouseover',function(event){
						var offset = $.jTableTool.getOffset(th, event).offsetX;
						if($th.outerWidth() - offset < 5) {
							$th.css("cursor", "col-resize").live('mousedown',function(event){
