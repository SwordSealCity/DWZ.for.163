...
(function($){
			...
			*/
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(e){
					...
					// 先获取input
					/*
					var $ck = $('td:first-child input',$tr);
					// 判断input是否存
					if($ck[0]){
						if($ck[0].checked){
							$ck[0].checked = false;
						...
						if($th.outerWidth() - offset < 5) {
							$th.css("cursor", "col-resize").mousedown(function(event){
								$(".resizeProxy", $grid).show().css({
									left: $.jTableTool.getRight(th)- $(".gridScroller", $grid).scrollLeft() + 10,
									top:$.jTableTool.getTop(th),
									height:$.jTableTool.getHeight(th,$grid),
									cursor:"col-resize"
								});
								$(".resizeMarker", $grid).show().css({
										left: $.jTableTool.getLeft(th) + 1 - $(".gridScroller", $grid).scrollLeft() + 10,
										top: $.jTableTool.getTop(th),
										height:$.jTableTool.getHeight(th,$grid)									
								});
								// 实现表格拖动变换宽度
	...
	$.jTableTool = {
		...
		resetW:function(tb1,tb2,oldW,newW){
