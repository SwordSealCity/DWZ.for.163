 ...
 * arTree --- @author Aramey
 * @role 在点击navTab选项的时候，控制左侧菜单效果。
 */
var arTree = {
	_treeBox: null,
	_lastTree: null,
	// 通过传递过来的$tab对象获取到对应的tabid，然后通过遍历检索左侧菜单
	// 检索到后就对之前选中的菜单进行效果移除，然后在遍历到的菜单添加效果
	goMenu:function($tab){
		...
		$('a', $tree).each(function(){
			if($(this).attr('href') == tabUrl){
				...
				$("div.selected", $tree).removeClass('selected');
				//$('>li', $tree).find('div').eq(0).addClass('selected');
			}
		});
	}
};
