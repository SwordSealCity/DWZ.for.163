...
var navTab = {
	...
	// 设置滚动显示的宽度 63是对应的margin值
	// margin的值是变动的，所以需要设置一个固定的值来进行计算
	...
	// 增加左侧菜单跟随动画 --- Aramey
	// 需要获取url参数进行对应的菜单指向			通过url进行实现
	// 首先获取左侧菜单target属性，建立相对的索引
	// 然后在点击的事件中添加对左侧菜单的控制
	// 再根据navTab的索引进行指向
	// 
	...
	// 修改tabs、MoreLi的标题，用于链接打开模式下或手动修改tabs的标题---Aramey
